...
) const
{
    ...
    {
        os << std::format(
            std::runtime_format(UtilGetResourceString(MA_STRING_COMMAND_HELP_COPYRIGHT)),
            L"2024",
    ...
    {
        os << std::format(
            std::runtime_format(UtilGetResourceString(MA_STRING_COMMAND_HELP_HINT_SUBCOMMAND)),
            std::format(
                L"{} [{}] {}",
                _name,
